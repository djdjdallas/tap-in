    ...
    const getDefaultAvatarUrl = async () => {
      try {
        const { data, error } = await supabase.storage
          .from("avatars")
          .createSignedUrl("default-avatar.png", 60 * 60); // 1 hour expiry
        ...
        {isLoading ? (
          ...
          <img
            ...
            onError={async (e) => {
              ...
              try {
                // If the current URL fails, try to get a signed URL
                if (profileData.avatar_url) {
                  const path = profileData.avatar_url.split("/").pop();
                  const { data, error } = await supabase.storage
                    .from("avatars")
                    .createSignedUrl(`${user.id}/${path}`, 60 * 60);
                ...
                // If signed URL fails or no avatar URL exists, use default
                e.target.src = defaultAvatar;

                // Update profile data to use default avatar
                setProfileData((prev) => ({
                  ...prev,
                  avatar_url: defaultAvatar,
                }));
